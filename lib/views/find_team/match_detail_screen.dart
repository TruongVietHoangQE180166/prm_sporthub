import 'package:flutter/material.dart';
import 'package:flutter_secure_storage/flutter_secure_storage.dart';

class MatchDetailScreen extends StatefulWidget {
  final Map<String, dynamic> matchData;

  const MatchDetailScreen({super.key, required this.matchData});

  @override
  State<MatchDetailScreen> createState() => _MatchDetailScreenState();
}

class _MatchDetailScreenState extends State<MatchDetailScreen> {
  final FlutterSecureStorage _secureStorage = const FlutterSecureStorage();
  String? _currentUserId;

  @override
  void initState() {
    super.initState();
    _loadCurrentUserId();
  }

  Future<void> _loadCurrentUserId() async {
    try {
      final userId = await _secureStorage.read(key: 'userId');
      setState(() {
        _currentUserId = userId;
      });
    } catch (e) {
      // Handle error silently
      setState(() {
        _currentUserId = null;
      });
    }
  }

  bool get _isOwner {
    if (_currentUserId == null) return false;
    return _currentUserId == widget.matchData['ownerId'];
  }

  bool get _isParticipant {
    if (_currentUserId == null) return false;
    if (_isOwner) return false; // Owner is not considered a participant
    
    try {
      final members = widget.matchData['members'];
      if (members is List) {
        return members.any(
          (member) => member is Map && member['userId'] == _currentUserId,
        );
      }
    } catch (e) {
      // If there's any error accessing members, return false
    }
    return false;
  }

  // Check if current user has a pending join request
  bool get _hasPendingRequest {
    if (_currentUserId == null) return false;
    try {
      final requests = widget.matchData['teamJoinRequest'];
      if (requests is List) {
        return requests.any(
          (request) => request is Map && 
              request['userId'] == _currentUserId && 
              request['status'] == 'PENDING',
        );
      }
    } catch (e) {
      // If there's any error accessing requests, return false
    }
    return false;
  }

  // Check if current user has a rejected request
  bool get _hasRejectedRequest {
    if (_currentUserId == null) return false;
    try {
      final requests = widget.matchData['teamJoinRequest'];
      if (requests is List) {
        return requests.any(
          (request) => request is Map && 
              request['userId'] == _currentUserId && 
              request['status'] == 'REJECTED',
        );
      }
    } catch (e) {
      // If there's any error accessing requests, return false
    }
    return false;
  }

  String get _joinButtonStatus {
    if (_isOwner) return 'Bạn là người tổ chức';
    if (_isParticipant) return 'Bạn đã tham gia';
    if (_hasPendingRequest) return 'Đang chờ xác nhận';
    
    // Check if match has ended with error handling
    try {
      final DateTime matchTime = DateTime.parse(widget.matchData['timeMatch']);
      final bool isMatchEnded = matchTime.isBefore(DateTime.now());
      if (isMatchEnded) return 'Đã kết thúc';
    } catch (e) {
      // If date parsing fails, assume match hasn't ended
    }
    
    // Check if team is full with null safety
    try {
      final int currentPlayers = (widget.matchData['members'] as List?)?.length ?? 0 + 1; // +1 for owner
      final bool isTeamFull = widget.matchData['maxPlayers'] != null ? 
          currentPlayers >= widget.matchData['maxPlayers'] : false;
      if (isTeamFull) return 'Đã đầy';
    } catch (e) {
      // If there's an error checking team size, continue
    }
    
    return 'Tham gia';
  }

  bool get _isJoinButtonEnabled {
    // Button is disabled if:
    // 1. User is the owner
    // 2. User is already a participant
    // 3. User has a pending request
    // 4. Match has ended
    // 5. Team is full
    
    // If user has rejected request, they can join again as a new user
    if (_hasRejectedRequest) return true;
    
    if (_isOwner || _isParticipant || _hasPendingRequest) return false;
    
    // Check if match has ended with error handling
    try {
      final DateTime matchTime = DateTime.parse(widget.matchData['timeMatch']);
      final bool isMatchEnded = matchTime.isBefore(DateTime.now());
      if (isMatchEnded) return false;
    } catch (e) {
      // If date parsing fails, assume match hasn't ended
    }
    
    // Check if team is full with null safety
    try {
      final int currentPlayers = (widget.matchData['members'] as List?)?.length ?? 0 + 1; // +1 for owner
      final bool isTeamFull = widget.matchData['maxPlayers'] != null ? 
          currentPlayers >= widget.matchData['maxPlayers'] : false;
      if (isTeamFull) return false;
    } catch (e) {
      // If there's an error checking team size, continue
    }
    
    return true;
  }

  // Function to get disabled button color based on user status
  Color _getDisabledButtonColor() {
    if (_isOwner) {
      return Colors.orange; // Orange for owner
    } else if (_isParticipant) {
      return Colors.red; // Red for participant
    } else if (_hasPendingRequest) {
      return Colors.orange; // Orange for pending request
    }
    
    // Check if match has ended with error handling
    try {
      final DateTime matchTime = DateTime.parse(widget.matchData['timeMatch']);
      final bool isMatchEnded = matchTime.isBefore(DateTime.now());
      if (isMatchEnded) {
        return Colors.red; // Red for ended match
      }
      
      // Check if team is full with null safety
      final int currentPlayers = (widget.matchData['members'] as List?)?.length ?? 0 + 1; // +1 for owner
      final bool isTeamFull = widget.matchData['maxPlayers'] != null ? 
          currentPlayers >= widget.matchData['maxPlayers'] : false;
      if (isTeamFull) {
        return Colors.grey; // Grey for full team
      }
    } catch (e) {
      // If there's any error, return default grey
    }
    
    return Colors.grey; // Default grey
  }

  @override
  Widget build(BuildContext context) {
    // Parse date with error handling
    DateTime? matchTime;
    String formattedDate = '';
    String formattedTime = '';
    
    try {
      matchTime = DateTime.parse(widget.matchData['timeMatch']);
      formattedDate = '${matchTime.day}/${matchTime.month}/${matchTime.year}';
      formattedTime = '${matchTime.hour.toString().padLeft(2, '0')}:${matchTime.minute.toString().padLeft(2, '0')}';
    } catch (e) {
      // Handle invalid date format
      formattedDate = 'Invalid date';
      formattedTime = '';
    }
    
    // Check if match has ended (only if matchTime is valid)
    final DateTime now = DateTime.now();
    final bool isMatchEnded = matchTime != null ? matchTime.isBefore(now) : false;
    
    // Check if team is full with null safety
    final int currentPlayers = (widget.matchData['members'] as List?)?.length ?? 0 + 1; // +1 for owner
    final bool isTeamFull = widget.matchData['maxPlayers'] != null ? 
        currentPlayers >= widget.matchData['maxPlayers'] : false;
    
    // Determine team status for badge
    String statusText = '';
    Color statusColor = Colors.grey;
    Color statusBgColor = Colors.grey.withOpacity(0.1);
    
    if (isMatchEnded) {
      statusText = 'Đã kết thúc';
      statusColor = Colors.red;
      statusBgColor = Colors.red.withOpacity(0.1);
    } else if (isTeamFull) {
      statusText = 'Đã đầy';
      statusColor = Colors.orange;
      statusBgColor = Colors.orange.withOpacity(0.1);
    } else {
      statusText = 'Đang tuyển';
      statusColor = const Color(0xFF7FD957);
      statusBgColor = const Color(0xFF7FD957).withOpacity(0.1);
    }
    
    // Get sport icon with null safety
    IconData sportIcon = Icons.sports;
    final sportName = widget.matchData['nameSport'];
    if (sportName != null) {
      switch(sportName) {
        case 'Football':
        case 'Bóng đá':
          sportIcon = Icons.sports_soccer;
          break;
        case 'Basketball':
        case 'Bóng rổ':
          sportIcon = Icons.sports_basketball;
          break;
        case 'Tennis':
          sportIcon = Icons.sports_tennis;
          break;
        default:
          sportIcon = Icons.sports;
      }
    }
    
    // Get level info with null safety
    final String level = widget.matchData['level'] ?? 'UNKNOWN';
    String levelText = 'Không xác định';
    Color levelColor = Colors.grey;
    Color levelBgColor = Colors.grey.withOpacity(0.1);
    
    switch(level) {
      case 'LOW':
        levelText = 'Mới chơi';
        levelColor = Colors.green;
        levelBgColor = Colors.green.withOpacity(0.1);
        break;
      case 'MEDIUM':
        levelText = 'Trung bình';
        levelColor = Colors.orange;
        levelBgColor = Colors.orange.withOpacity(0.1);
        break;
      case 'HIGH':
        levelText = 'Chuyên nghiệp';
        levelColor = Colors.red;
        levelBgColor = Colors.red.withOpacity(0.1);
        break;
    }
    
    return Scaffold(
      backgroundColor: const Color(0xFFF5F7FA),
      appBar: null,
      body: SafeArea(
        child: Column(
          children: [
            // Custom header
            Container(
              decoration: BoxDecoration(
                gradient: LinearGradient(
                  begin: Alignment.topLeft,
                  end: Alignment.bottomRight,
                  colors: [
                    const Color(0xFF7FD957),
                    const Color(0xFF7FD957).withOpacity(0.85),
                  ],
                ),
                boxShadow: [
                  BoxShadow(
                    color: const Color(0xFF7FD957).withOpacity(0.25),
                    blurRadius: 15,
                    offset: const Offset(0, 5),
                  ),
                ],
              ),
              child: Padding(
                padding: const EdgeInsets.fromLTRB(20, 16, 20, 24),
                child: Row(
                  children: [
                    Container(
                      decoration: BoxDecoration(
                        color: Colors.white.withOpacity(0.25),
                        borderRadius: BorderRadius.circular(14),
                        border: Border.all(
                          color: Colors.white.withOpacity(0.3),
                          width: 1,
                        ),
                      ),
                      child: IconButton(
                        icon: const Icon(Icons.arrow_back_ios_new,
                            color: Colors.white, size: 20),
                        onPressed: () => Navigator.of(context).pop(),
                      ),
                    ),
                    const SizedBox(width: 16),
                    const Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(
                          'Chi tiết trận đấu',
                          style: TextStyle(
                            fontSize: 24,
                            fontWeight: FontWeight.w800,
                            color: Colors.white,
                            letterSpacing: 0.5,
                          ),
                        ),
                        SizedBox(height: 2),
                        Text(
                          'Thông tin chi tiết trận đấu',
                          style: TextStyle(
                            fontSize: 13,
                            fontWeight: FontWeight.w500,
                            color: Colors.white70,
                            letterSpacing: 0.2,
                          ),
                        ),
                      ],
                    ),
                  ],
                ),
              ),
            ),
            
            // Match details content
            Expanded(
              child: SingleChildScrollView(
                padding: const EdgeInsets.all(20),
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    // Match header card
                    Container(
                      width: double.infinity,
                      decoration: BoxDecoration(
                        color: Colors.white,
                        borderRadius: BorderRadius.circular(20),
                        boxShadow: [
                          BoxShadow(
                            color: Colors.black.withOpacity(0.06),
                            blurRadius: 16,
                            offset: const Offset(0, 4),
                          ),
                        ],
                      ),
                      child: Padding(
                        padding: const EdgeInsets.all(20),
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            // Sport icon and match name
                            Row(
                              children: [
                                Container(
                                  padding: const EdgeInsets.all(12),
                                  decoration: BoxDecoration(
                                    color: const Color(0xFF7FD957).withOpacity(0.15),
                                    borderRadius: BorderRadius.circular(12),
                                  ),
                                  child: Icon(
                                    sportIcon,
                                    color: const Color(0xFF7FD957),
                                    size: 24,
                                  ),
                                ),
                                const SizedBox(width: 14),
                                Expanded(
                                  child: Column(
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    children: [
                                      // Match name with icon
                                      Row(
                                        children: [
                                          const Icon(
                                            Icons.sports_soccer,
                                            color: Color(0xFF7FD957),
                                            size: 16,
                                          ),
                                          const SizedBox(width: 6),
                                          Expanded(
                                            child: Text(
                                              widget.matchData['nameMatch'] ?? 'Unknown Match',
                                              style: const TextStyle(
                                                fontSize: 18,
                                                fontWeight: FontWeight.w700,
                                                color: Colors.black87,
                                                letterSpacing: 0.2,
                                              ),
                                            ),
                                          ),
                                        ],
                                      ),
                                      const SizedBox(height: 4),
                                      // Organizer name with icon
                                      Row(
                                        children: [
                                          const Icon(
                                            Icons.person,
                                            color: Colors.grey,
                                            size: 14,
                                          ),
                                          const SizedBox(width: 6),
                                          Text(
                                            widget.matchData['ownerName'] ?? 'Unknown Organizer',
                                            style: TextStyle(
                                              fontSize: 14,
                                              fontWeight: FontWeight.w500,
                                              color: Colors.grey[600],
                                            ),
                                          ),
                                        ],
                                      ),
                                    ],
                                  ),
                                ),
                                // Badges column
                                Column(
                                  crossAxisAlignment: CrossAxisAlignment.end,
                                  children: [
                                    // Level badge
                                    Container(
                                      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                                      decoration: BoxDecoration(
                                        color: levelBgColor,
                                        borderRadius: BorderRadius.circular(20),
                                      ),
                                      child: Text(
                                        levelText,
                                        style: TextStyle(
                                          fontSize: 12,
                                          fontWeight: FontWeight.w600,
                                          color: levelColor,
                                        ),
                                      ),
                                    ),
                                    const SizedBox(height: 6),
                                    // Status badge
                                    Container(
                                      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                                      decoration: BoxDecoration(
                                        color: statusBgColor,
                                        borderRadius: BorderRadius.circular(20),
                                      ),
                                      child: Text(
                                        statusText,
                                        style: TextStyle(
                                          fontSize: 12,
                                          fontWeight: FontWeight.w600,
                                          color: statusColor,
                                        ),
                                      ),
                                    ),
                                  ],
                                ),
                              ],
                            ),
                            const SizedBox(height: 20),
                            
                            // Match details
                            Container(
                              padding: const EdgeInsets.all(16),
                              decoration: BoxDecoration(
                                color: const Color(0xFF7FD957).withOpacity(0.05),
                                borderRadius: BorderRadius.circular(12),
                              ),
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  // Location
                                  Row(
                                    children: [
                                      Container(
                                        padding: const EdgeInsets.all(8),
                                        decoration: BoxDecoration(
                                          color: const Color(0xFF7FD957).withOpacity(0.1),
                                          borderRadius: BorderRadius.circular(8),
                                        ),
                                        child: const Icon(
                                          Icons.location_on,
                                          color: Color(0xFF7FD957),
                                          size: 18,
                                        ),
                                      ),
                                      const SizedBox(width: 12),
                                      Expanded(
                                        child: Text(
                                          widget.matchData['location'] ?? 'Unknown Location',
                                          style: const TextStyle(
                                            fontSize: 14,
                                            color: Colors.black87,
                                          ),
                                        ),
                                      ),
                                    ],
                                  ),
                                  const SizedBox(height: 16),
                                  
                                  // Time
                                  Row(
                                    children: [
                                      Container(
                                        padding: const EdgeInsets.all(8),
                                        decoration: BoxDecoration(
                                          color: const Color(0xFF7FD957).withOpacity(0.1),
                                          borderRadius: BorderRadius.circular(8),
                                        ),
                                        child: const Icon(
                                          Icons.access_time,
                                          color: Color(0xFF7FD957),
                                          size: 18,
                                        ),
                                      ),
                                      const SizedBox(width: 12),
                                      Text(
                                        '$formattedDate lúc $formattedTime',
                                        style: const TextStyle(
                                          fontSize: 14,
                                          color: Colors.black87,
                                        ),
                                      ),
                                      const SizedBox(width: 8),
                                      // Show "Đã kết thúc" badge if match has ended
                                      if (isMatchEnded)
                                        Container(
                                          padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                                          decoration: BoxDecoration(
                                            color: Colors.red.withOpacity(0.1),
                                            borderRadius: BorderRadius.circular(12),
                                          ),
                                          child: const Text(
                                            'Đã kết thúc',
                                            style: TextStyle(
                                              fontSize: 12,
                                              fontWeight: FontWeight.w600,
                                              color: Colors.red,
                                            ),
                                          ),
                                        ),
                                    ],
                                  ),
                                  const SizedBox(height: 16),
                                  
                                  // Player count
                                  Row(
                                    children: [
                                      Container(
                                        padding: const EdgeInsets.all(8),
                                        decoration: BoxDecoration(
                                          color: const Color(0xFF7FD957).withOpacity(0.1),
                                          borderRadius: BorderRadius.circular(8),
                                        ),
                                        child: const Icon(
                                          Icons.group,
                                          color: Color(0xFF7FD957),
                                          size: 18,
                                        ),
                                      ),
                                      const SizedBox(width: 12),
                                      Text(
                                        'Số người: ${widget.matchData['members'] != null ? (widget.matchData['members'] as List).length + 1 : 1}/${widget.matchData['maxPlayers'] ?? 'N/A'}',
                                        style: const TextStyle(
                                          fontSize: 14,
                                          color: Colors.black87,
                                        ),
                                      ),
                                      const SizedBox(width: 8),
                                      // Show "Đã đầy" badge if team is full
                                      if (isTeamFull && !isMatchEnded)
                                        Container(
                                          padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                                          decoration: BoxDecoration(
                                            color: Colors.orange.withOpacity(0.1),
                                            borderRadius: BorderRadius.circular(12),
                                          ),
                                          child: const Text(
                                            'Đã đầy',
                                            style: TextStyle(
                                              fontSize: 12,
                                              fontWeight: FontWeight.w600,
                                              color: Colors.orange,
                                            ),
                                          ),
                                        ),
                                    ],
                                  ),
                                ],
                              ),
                            ),
                          ],
                        ),
                      ),
                    ),
                    
                    const SizedBox(height: 20),
                    
                    // Description section
                    if (widget.matchData['descriptionMatch'] != null && widget.matchData['descriptionMatch'].isNotEmpty)
                      Container(
                        width: double.infinity,
                        decoration: BoxDecoration(
                          color: Colors.white,
                          borderRadius: BorderRadius.circular(20),
                          boxShadow: [
                            BoxShadow(
                              color: Colors.black.withOpacity(0.06),
                              blurRadius: 16,
                              offset: const Offset(0, 4),
                            ),
                          ],
                        ),
                        child: Padding(
                          padding: const EdgeInsets.all(20),
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              const Text(
                                'Mô tả',
                                style: TextStyle(
                                  fontSize: 16,
                                  fontWeight: FontWeight.w600,
                                  color: Colors.black87,
                                ),
                              ),
                              const SizedBox(height: 12),
                              Text(
                                widget.matchData['descriptionMatch'] ?? '',
                                style: TextStyle(
                                  fontSize: 14,
                                  color: Colors.grey[700],
                                ),
                              ),
                            ],
                          ),
                        ),
                      ),
                    
                    const SizedBox(height: 20),
                    
                    // Contact information
                    Container(
                      width: double.infinity,
                      decoration: BoxDecoration(
                        color: Colors.white,
                        borderRadius: BorderRadius.circular(20),
                        boxShadow: [
                          BoxShadow(
                            color: Colors.black.withOpacity(0.06),
                            blurRadius: 16,
                            offset: const Offset(0, 4),
                          ),
                        ],
                      ),
                      child: Padding(
                        padding: const EdgeInsets.all(20),
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text(
                              'Thông tin liên hệ',
                              style: TextStyle(
                                fontSize: 16,
                                fontWeight: FontWeight.w600,
                                color: Colors.black87,
                              ),
                            ),
                            const SizedBox(height: 16),
                            
                            // Phone number
                            Row(
                              children: [
                                Container(
                                  padding: const EdgeInsets.all(8),
                                  decoration: BoxDecoration(
                                    color: const Color(0xFF7FD957).withOpacity(0.1),
                                    borderRadius: BorderRadius.circular(8),
                                  ),
                                  child: const Icon(
                                    Icons.phone,
                                    color: Color(0xFF7FD957),
                                    size: 18,
                                  ),
                                ),
                                const SizedBox(width: 12),
                                Text(
                                  widget.matchData['numberPhone'] ?? 'Chưa cung cấp',
                                  style: const TextStyle(
                                    fontSize: 14,
                                    color: Colors.black87,
                                  ),
                                ),
                              ],
                            ),
                            const SizedBox(height: 16),
                            
                            // Facebook link
                            Row(
                              children: [
                                Container(
                                  padding: const EdgeInsets.all(8),
                                  decoration: BoxDecoration(
                                    color: const Color(0xFF7FD957).withOpacity(0.1),
                                    borderRadius: BorderRadius.circular(8),
                                  ),
                                  child: const Icon(
                                    Icons.facebook,
                                    color: Color(0xFF7FD957),
                                    size: 18,
                                  ),
                                ),
                                const SizedBox(width: 12),
                                Expanded(
                                  child: Text(
                                    widget.matchData['linkFacebook'] ?? 'Chưa cung cấp',
                                    style: const TextStyle(
                                      fontSize: 14,
                                      color: Colors.black87,
                                    ),
                                    maxLines: 2,
                                    overflow: TextOverflow.ellipsis,
                                  ),
                                ),
                              ],
                            ),
                          ],
                        ),
                      ),
                    ),
                    
                    const SizedBox(height: 20),
                    
                    // Participants section
                    Container(
                      width: double.infinity,
                      decoration: BoxDecoration(
                        color: Colors.white,
                        borderRadius: BorderRadius.circular(20),
                        boxShadow: [
                          BoxShadow(
                            color: Colors.black.withOpacity(0.06),
                            blurRadius: 16,
                            offset: const Offset(0, 4),
                          ),
                        ],
                      ),
                      child: Padding(
                        padding: const EdgeInsets.all(20),
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text(
                              'Người tham gia',
                              style: TextStyle(
                                fontSize: 16,
                                fontWeight: FontWeight.w600,
                                color: Colors.black87,
                              ),
                            ),
                            const SizedBox(height: 16),
                            
                            // Owner
                            Container(
                              padding: const EdgeInsets.all(12),
                              decoration: BoxDecoration(
                                color: const Color(0xFF7FD957).withOpacity(0.1),
                                borderRadius: BorderRadius.circular(12),
                              ),
                              child: Row(
                                children: [
                                  Container(
                                    padding: const EdgeInsets.all(6),
                                    decoration: BoxDecoration(
                                      color: const Color(0xFF7FD957),
                                      borderRadius: BorderRadius.circular(20),
                                    ),
                                    child: const Icon(
                                      Icons.star,
                                      color: Colors.white,
                                      size: 16,
                                    ),
                                  ),
                                  const SizedBox(width: 12),
                                  Expanded(
                                    child: Column(
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: [
                                        Text(
                                          '${widget.matchData['ownerName'] ?? 'Unknown'} (Người tổ chức)',
                                          style: const TextStyle(
                                            fontSize: 14,
                                            fontWeight: FontWeight.w500,
                                            color: Color(0xFF7FD957),
                                          ),
                                        ),
                                        const SizedBox(height: 4),
                                        // Show owner email if available
                                        if (widget.matchData['members'] != null && 
                                            (widget.matchData['members'] as List).isNotEmpty && 
                                            (widget.matchData['members'] as List).any((member) => 
                                                member is Map && 
                                                member['userId'] == widget.matchData['ownerId'] && 
                                                member['email'] != null))
                                          Text(
                                            (widget.matchData['members'] as List)
                                                .firstWhere(
                                                  (member) => member is Map && member['userId'] == widget.matchData['ownerId'],
                                                  orElse: () => {'email': ''}
                                                )['email'].toString(),
                                            style: const TextStyle(
                                              fontSize: 12,
                                              color: Colors.grey,
                                            ),
                                          ),
                                      ],
                                    ),
                                  ),
                                ],
                              ),
                            ),
                            
                            const SizedBox(height: 12),
                            
                            // Other participants
                            if (widget.matchData['members'] == null || (widget.matchData['members'] as List).isEmpty)
                              Container(
                                padding: const EdgeInsets.all(12),
                                decoration: BoxDecoration(
                                  color: Colors.grey.withOpacity(0.1),
                                  borderRadius: BorderRadius.circular(12),
                                ),
                                child: const Text(
                                  'Chưa có người tham gia khác',
                                  style: TextStyle(
                                    fontSize: 14,
                                    color: Colors.grey,
                                  ),
                                ),
                              )
                            else
                              Column(
                                children: (widget.matchData['members'] as List).map<Widget>((member) {
                                  // Skip owner as they're displayed separately
                                  if (member is Map && member['userId'] == widget.matchData['ownerId']) {
                                    return const SizedBox.shrink();
                                  }
                                  
                                  return Container(
                                    margin: const EdgeInsets.only(top: 8),
                                    padding: const EdgeInsets.all(12),
                                    decoration: BoxDecoration(
                                      color: Colors.grey.withOpacity(0.05),
                                      borderRadius: BorderRadius.circular(12),
                                      border: Border.all(
                                        color: Colors.grey.withOpacity(0.1),
                                      ),
                                    ),
                                    child: Row(
                                      children: [
                                        Container(
                                          padding: const EdgeInsets.all(6),
                                          decoration: BoxDecoration(
                                            color: Colors.grey.withOpacity(0.2),
                                            borderRadius: BorderRadius.circular(20),
                                          ),
                                          child: const Icon(
                                            Icons.person,
                                            color: Colors.grey,
                                            size: 16,
                                          ),
                                        ),
                                        const SizedBox(width: 12),
                                        Expanded(
                                          child: Column(
                                            crossAxisAlignment: CrossAxisAlignment.start,
                                            children: [
                                              Text(
                                                member is Map ? (member['username'] ?? 'Người dùng') : 'Người dùng',
                                                style: const TextStyle(
                                                  fontSize: 14,
                                                  color: Colors.black87,
                                                ),
                                              ),
                                              const SizedBox(height: 4),
                                              // Show member email if available
                                              if (member is Map && member['email'] != null)
                                                Text(
                                                  member['email'].toString(),
                                                  style: const TextStyle(
                                                    fontSize: 12,
                                                    color: Colors.grey,
                                                  ),
                                                ),
                                            ],
                                          ),
                                        ),
                                      ],
                                    ),
                                  );
                                }).toList(),
                              ),
                          ],
                        ),
                      ),
                    ),
                    
                    const SizedBox(height: 24),
                    
                    // Action buttons
                    Row(
                      children: [
                        Expanded(
                          child: OutlinedButton(
                            onPressed: () {
                              Navigator.of(context).pop();
                            },
                            style: OutlinedButton.styleFrom(
                              side: const BorderSide(color: Color(0xFF7FD957)),
                              shape: RoundedRectangleBorder(
                                borderRadius: BorderRadius.circular(12),
                              ),
                              padding: const EdgeInsets.symmetric(vertical: 16),
                            ),
                            child: const Text(
                              'Quay lại',
                              style: TextStyle(
                                fontSize: 14,
                                fontWeight: FontWeight.w600,
                                color: Color(0xFF7FD957),
                              ),
                            ),
                          ),
                        ),
                        // Show "Hủy yêu cầu" button when user has pending request (red and enabled)
                        if (_hasPendingRequest) ...[
                          const SizedBox(width: 12),
                          Expanded(
                            child: ElevatedButton(
                              onPressed: () {
                                // TODO: Implement cancel join request functionality
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content: Text('Chức năng hủy yêu cầu tham gia sẽ được triển khai sau'),
                                    backgroundColor: Colors.red,
                                  ),
                                );
                              },
                              style: ElevatedButton.styleFrom(
                                backgroundColor: Colors.red, // Red color as requested
                                shape: RoundedRectangleBorder(
                                  borderRadius: BorderRadius.circular(12),
                                ),
                                padding: const EdgeInsets.symmetric(vertical: 16),
                              ),
                              child: const Text(
                                'Hủy yêu cầu',
                                style: TextStyle(
                                  fontSize: 14,
                                  fontWeight: FontWeight.w600,
                                  color: Colors.white,
                                ),
                              ),
                            ),
                          ),
                        ] else if (_isJoinButtonEnabled) ...[
                          const SizedBox(width: 12),
                          Expanded(
                            child: ElevatedButton(
                              onPressed: () {
                                // TODO: Implement join team functionality
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content: Text('Chức năng tham gia trận đấu sẽ được triển khai sau'),
                                    backgroundColor: Color(0xFF7FD957),
                                  ),
                                );
                              },
                              style: ButtonStyle(
                                backgroundColor: MaterialStateProperty.resolveWith<Color>(
                                  (Set<MaterialState> states) {
                                    if (states.contains(MaterialState.disabled)) {
                                      return _getDisabledButtonColor();
                                    }
                                    return const Color(0xFF7FD957);
                                  },
                                ),
                                shape: MaterialStateProperty.all<RoundedRectangleBorder>(
                                  RoundedRectangleBorder(
                                    borderRadius: BorderRadius.circular(12),
                                  ),
                                ),
                                padding: MaterialStateProperty.all<EdgeInsets>(
                                  const EdgeInsets.symmetric(vertical: 16),
                                ),
                              ),
                              child: Text(
                                _joinButtonStatus,
                                style: const TextStyle(
                                  fontSize: 14,
                                  fontWeight: FontWeight.w600,
                                  color: Colors.white,
                                ),
                              ),
                            ),
                          ),
                        ],
                      ],
                    ),
                  ],
                ),
              ),
            ),
          ],
        ),
      ),
    );
  }
}